...
class roomCreationHandler extends serverEventHandlerBase{
    ...
    async handle(message: string, sourceSocket: Socket): Promise<void> {
        let options = <IRoomCreationPackege> await ObjectsSerializeUtil.instance.parse(message);
        ...
        if(!isConnectedToAnyRoom){
            ...
            sourceSocket.emit(responseEventsList.roomCreated, await ObjectsSerializeUtil.instance.serialize({createdRoomId: guid}));
        }
        else{
